...
#define EEPROM_I2C_FREQ_HZ 400000 
...
esp_err_t eeprom_init(i2c_master_bus_handle_t bus_handle) {
    if (bus_handle == NULL) {
        ESP_LOGE(TAG, "Invalid I2C bus handle passed to EEPROM!");
    ...
    esp_err_t ret = i2c_master_bus_add_device(bus_handle, &dev_cfg, &eeprom_dev_handle);
    if (ret == ESP_OK) {
        ESP_LOGI(TAG, "EEPROM 24LC128 registered at 0x%02X", EEPROM_I2C_ADDR);
    } else {
        ESP_LOGE(TAG, "Failed to add EEPROM to I2C bus: %s", esp_err_to_name(ret));
    }
    return ret;
}
...
esp_err_t eeprom_load_settings(app_settings_t *settings) {
    ...
    // 1. Перевірка результату читання та магічного числа
    ...
    // 3. Завантаження дефолтних налаштувань у разі помилки або зміни магічного числа
    if (valid) {
        ...
        ESP_LOGW(TAG, "EEPROM empty, corrupted or format changed. Writing default profile...");
        ...
        settings->adau_main_volume = 10; // Дефолтна загальна гучність
        
        // Зміщення: 24 означає 0 dB (діапазон -12.0 до +12.0)
        memset(settings->adau_vol_ch, 24, sizeof(settings->adau_vol_ch)); 
        memset(settings->adau_distances, 0, sizeof(settings->adau_distances)); // Дистанції 0
        
        settings->adau_eq_preset = 0;     // Studio Flat
        settings->adau_surround_mode = 0; // Music Mode
...
bool settings_commit_check(void) {
    ...
    if (memcmp(&current_snapshot, &g_saved_settings, offsetof(app_settings_t, crc8)) != 0) {

...
#define IS_INPUT_SIGNAL_TIMEOUT 10 // ~10 секунд (перевірка раз на секунду)

// [ФІКС] Додано volatile для безпечного доступу з різних задач
volatile uint8_t is_input_sig_flag = 0;
...
void is_input_signal(uint8_t *src, size_t size) {
    ...
    if (frame_skip++ > 46) { // Обчислення приблизно раз на 1 секунду
        frame_skip = 0;
        
        // [ФІКС] Локальна змінна для суми поточного кадру, щоб уникнути переповнення
        ...
        uint32_t current_avg = current_sum / size;

        // Якщо середній рівень шуму/сигналу змінився - музика грає або змінилася
        if (current_avg != old_state) {
            old_state = current_avg;
            old_state_count = 0;
            ...
            // Якщо рівень стабільний (тиша або статичний шум) - рахуємо таймаут
...
void get_signal_process(void *argv) {
    ...
    for (;;) {
        ...
        if (ret == ESP_OK) {
            ...
            vTaskDelay(pdMS_TO_TICKS(100)); // [ФІКС] Зменшено затримку при помилці, щоб не "вішати" задачу на секунду
        }
    }
}
...
void analiz_process(void *argv) {
    ...
    for (;;) {
        if (xSemaphoreTake(fft_process_sem, portMAX_DELAY) == pdTRUE) {
            ...
            // Mapping to Display Columns (Left Channel)
            ...
            // Mapping to Display Columns (Right Channel)
            for (size_t i = 0; i < COLUM_SIZE / 2; i++) {
                if (y2_cf[i * 2 + 1] < 0) {
            ...
            // Send data to UI Task via Queue
            xQueueSend(g_fft_process_result_queue, colum, 0);
            
            // Check for silence / active music
...
void analizator_init(void) {
    ...
    // [ФІКС] Піднято пріоритет з 0 (tskIDLE_PRIORITY) до 2, щоб уникнути візуальних "підвисань" аналізатора
    xTaskCreate(&get_signal_process, "get_signal_process", 4096, NULL, 2, &read_signal_task_handle);
    xTaskCreate(&analiz_process, "analiz_process", 4096, NULL, 2, &fft_task_handle);
}
